...
function HomeView({ user, onLogin, isDeadlinePassed, setView }) {
  ...
  const handleRegister = async () => {
    ...
    const { data, error: err } = await supabase.from("participants").insert({ name: name.trim(), pin_hash, is_admin: false }).select().single();
  ...
  const handleLogin = async () => {
    ...
    setBusy(true); setError("");
    const { data, error: err } = await supabase.from("participants").select("*").ilike("name", name.trim()).maybeSingle();
  ...
  if (user) return (
    <div>
      <div className="hero">
        ...
        <div className="hero-deadline">{isDeadlinePassed ? <>🔒 Tips are <strong>locked</strong></> : <>📅 Deadline: <strong>{deadlineStr} (UTC)</strong></>}</div>
      </div>
      <div className="card">
        <div className="card-title">👋 Welcome back, {user.name}!</div>
        <p className="text-muted" style={{marginBottom:16}}>Head to <strong>Leaderboard</strong> to see rankings or <strong>My Tips</strong> to view your picks.</p>
  ...
  return (
    <div>
      <div className="hero">
        <h1>WC 2026</h1>
        <h2>World Cup Tipping Competition</h2>
        <div className="hero-deadline">{isDeadlinePassed ? <>🔒 Tips are <strong>locked</strong></> : <>📅 Tips deadline: <strong>{deadlineStr} (UTC)</strong></>}</div>
      </div>
      <div className="auth-grid">
        ...
        <div className="card">
          ...
          <div className="section-intro">
            ...
            Pick the exact score for every match. Correct result (win/draw/loss) = odds points. Exact score = +5 bonus.<br/><br/>
            ...
            <em style={{color:"var(--text3)"}}>All tips locked before June 11. No changes after deadline.</em>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{paddingBottom:isDeadlinePassed?0:80}}>
      <div style={{margin:"24px 0 16px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
        ...
        {isDeadlinePassed
          ? <span className="alert alert-info" style={{margin:0}}>🔒 Deadline passed — view only</span>
                ...
                <div>
                  <div className="match-score">
                    {isDeadlinePassed ? (
                      ...
                      <><input className="score-input" type="number" min="0" max="20" value={tip.home??""} onChange={e=>updateTip(m.id,"home",e.target.value)} placeholder="0"/><span className="score-sep">-</span><input className="score-input" type="number" min="0" max="20" value={tip.away??""} onChange={e=>updateTip(m.id,"away",e.target.value)} placeholder="0"/></>
                    )}
                  </div>
            ...
            return (
              <div key={r.key} className="qualifier-section card">
                ...
                <div className="team-grid">
                  {ALL_TEAMS.map(team => (
                    <div key={team} className={`team-chip${selected.includes(team)?" selected":""}${isDeadlinePassed?" locked":""}`} onClick={()=>toggleQualTeam(r.key,team,r.count)}>{team}</div>
                  ))}
                </div>
...
function AdminView() {
  ...
  const saveResult = async (match) => {
    ...
    for (const t of (tips||[])) {
      ...
      let pts = 0;
      if (tipRes===actualRes) pts = tipRes==="1"?match.odds_home:tipRes==="2"?match.odds_away:match.odds_draw;
  ...
  return (
    <div>
      ...
      {tab==="results" && (
        <div>
          <p className="section-intro">Enter final scores. Points calculated automatically for all participants.</p>
          {groups.map(g=>(
            <div key={g}>
              <div className="group-header">GROUP {g}</div>
              {matches.filter(m=>m.group_name===g).map(m=>(
                <div key={m.id} className="result-row">
                  ...
                  <input className="result-input" type="number" min="0" max="20" placeholder="0" value={results[m.id]?.home??""} onChange={e=>setResults(r=>({...r,[m.id]:{...r[m.id],home:e.target.value}}))} />
                  <span className="result-sep">-</span>
                  <input className="result-input" type="number" min="0" max="20" placeholder="0" value={results[m.id]?.away??""} onChange={e=>setResults(r=>({...r,[m.id]:{...r[m.id],away:e.target.value}}))} />
                  <button className="btn-sm btn-green" onClick={()=>saveResult(m)} disabled={saving[m.id]}>{saving[m.id]?"...":"Save"}</button>
        ...
        <div>
          ...
          {ROUNDS.map(r=>(
            <div key={r.key} className="card qualifier-section">
              ...
              <div className="team-grid">
                {ALL_TEAMS.map(team=>(
                  <div key={team} className={`team-chip${(qualResults[r.key]||[]).includes(team)?" selected":""}`} onClick={()=>toggleQualResult(r.key,team,r.count)}>{team}</div>
                ))}
              </div>
              <div style={{display:"flex",alignItems:"center",gap:12,marginTop:12}}>
                <span className="count-badge">{(qualResults[r.key]||[]).length}/{r.count} selected</span>
                <button className="btn-sm btn-green" onClick={()=>saveQualResults(r.key)} disabled={saving[r.key]}>{saving[r.key]?"Saving...":"Save & Score"}</button>
      ...
      {tab==="champion" && (
        <div className="card">
          ...
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <button className="btn btn-primary" style={{maxWidth:160}} onClick={saveChampResult} disabled={saving.champ||!champResult}>{saving.champ?"Saving...":"Save & Score"}</button>
          ...
          {participants.map(p=>(
            <div key={p.id} className="participant-row">
              ...
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <input className="form-input" type="number" inputMode="numeric" maxLength={4} placeholder="New PIN" style={{width:100,padding:"6px 10px",fontSize:14}} value={newPin[p.id]||""} onChange={e=>setNewPin(n=>({...n,[p.id]:e.target.value.replace(/\D/g,"").slice(0,4)}))} />
